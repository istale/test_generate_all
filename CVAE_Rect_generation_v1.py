...
class RectangleDataset(Dataset):
    def __init__(self, num_samples=10000):
        ...
        for _ in range(num_samples):
            # 確保 w/h 或 h/w 至少大於 5
            while True:
                w_g = np.random.uniform(50, 500)  # given width
...
def train_cvae():
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}, Loss: {total_loss / len(dataloader):.8f}")
...
def generate_rectangle(model, given_rect):
    model.eval()
    given_rect = [given_rect[0] / 500, given_rect[1] / 500, given_rect[2] / 300, given_rect[3] / 300]
    ...
    generated_rect[0] *= 500
    generated_rect[1] *= 500
    generated_rect[2] *= 300
    generated_rect[3] *= 300
